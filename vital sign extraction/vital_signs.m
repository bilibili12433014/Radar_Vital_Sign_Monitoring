...
Fs=6.5e6;             %ADC采样率 ：6.5Msps            
c=3*1e8;            %光速
ts=numADCSamples/Fs;    %ADC采样时间
slope=49.97e12;                 %调频斜率 ：~64.985MHz/us
B_valid =ts*slope;                %有效带宽：3.25GHz
detaR=c/(2*B_valid);          %距离分辨率：4.615cm（range-bin）ΔR=0.04615
startFreq = 60e9;              %起始频率 ：60.25GHz
lambda=c/startFreq;              % 雷达信号波长：  λ=c/f0 = 5mm

%% 读取Bin文件（数据预处理）
% Filename = 'adc_data20.bin';       %解析不同的数据修改该文件名即可
Filename = 'huxi2_Raw_0.bin';
...
angle_fft_last = angle_fft(:,max_num);%1024个chrip的某列rangebin的相位 
% 提取相位信号（原始）
figure;
plot(angle_fft_last);
